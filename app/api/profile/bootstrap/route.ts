...
export async function POST() {
  ...
  const { error: musclesUpsertError } = await supabase.from("muscle_stats").upsert(
  ...
  const { error: initialContentError } = await supabase.rpc("fn_grant_initial_content", { p_user_id: user.id });
