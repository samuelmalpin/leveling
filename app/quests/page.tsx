...
import { AchievementChainList } from "@/components/features/achievement-chain-list";

export default async function QuestsPage() {
  const { supabase, user } = await requireUser();
  const today = new Date().toISOString().slice(0, 10);

  const [{ data: mainQuests }, { data: weeklyRows }, { data: microRows }, { data: chainRows }] = await Promise.all([
    supabase
      .from("quest_progress")
    ...
    supabase
      ...
      .select("id, progress_value, status, achievement_chains(name, description, milestone_target, xp_reward)")
      .eq("user_id", user.id)
      .order("updated_at", { ascending: false })
  ]);
  ...
  return (
    <div className="space-y-3">
      ...
      <AchievementChainList initialRows={(chainRows ?? []) as never[]} />
